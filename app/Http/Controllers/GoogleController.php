...
class GoogleController extends Controller
{
    ...
    public function callback(Request $request, GoogleDriveService $google)
    {
        ...
        if (isset($token['error'])) {
            return redirect()->route('dashboard')->with('error', $token['error_description'] ?? 'Gagal mengambil token Google.');
        }
        ...
        return redirect()->route('dashboard')->with('success', 'Akun Google Drive berhasil ditambahkan ke grup: ' . $account->group?->name);
    }
}